...
class AlienInvasion:
    ...
    def _check_play_button(self, mouse_pos):
        """Sart a new game when the player clicks Play"""
    ...
    def _check_bullet_alien_collisions(self):
        ...
        if not self.aliens:
            # Destroye existing bullets and create new fleet
            self.bullets.empty()
            self._create_fleet()
            self.settings.increases_speed()
    ...
    def _check_aliens_bottom(self):
        ...
        for alien in self.aliens.sprites():
            if alien.rect.bottom >= screen_rect.bottom:
                #Treat this the same has if the ship was hit
                self._ship_hit()
                break
